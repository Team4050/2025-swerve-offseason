...
```mermaid
graph TD
    subgraph ROBOT_CODE["ROBOT CODE LAYER"]
        CMD["CommandSwerveDrivetrain (Subsystem)<br/>- WPILib Command-based wrapper<br/>- Integrates with autonomous commands<br/>- Handles simulation and telemetry"]
        TUNER["TunerSwerveDrivetrain<br/>- Generated by Tuner X<br/>- Extends SwerveDrivetrain base class<br/>- Contains robot-specific constants"]
        CMD --> TUNER
    end
    
    subgraph PHOENIX_API["PHOENIX 6 API LAYER"]
        BASE["SwerveDrivetrain Base Class<br/>- Core swerve logic and control<br/>- Odometry management<br/>- Module coordination"]
        REQUEST["SwerveRequest API<br/>- FieldCentric, RobotCentric<br/>- FieldCentricFacingAngle<br/>- SwerveDriveBrake, Idle"]
        MODULES["SwerveModule Classes<br/>- Individual module control (×4)<br/>- Drive + Steer motor coordination<br/>- Encoder feedback integration"]
        BASE --> REQUEST
        REQUEST --> MODULES
    end
    
    subgraph HARDWARE["HARDWARE LAYER"]
        DRIVE_MOTORS["TalonFX Drive Motors<br/>FL, FR, BL, BR"]
        STEER_MOTORS["TalonFX Steer Motors<br/>FL, FR, BL, BR"]
        ENCODERS["CANcoder Encoders<br/>FL, FR, BL, BR"]
        IMU["Pigeon 2.0 (IMU)"]
        CAN["Connected via CAN Bus<br/>(rio or CANivore name)"]
        DRIVE_MOTORS -.-> CAN
        STEER_MOTORS -.-> CAN
        ENCODERS -.-> CAN
        IMU -.-> CAN
    ...
    TUNER --> BASE
    MODULES --> DRIVE_MOTORS
    MODULES --> STEER_MOTORS
    MODULES --> ENCODERS
    BASE --> IMU
    
    style ROBOT_CODE fill:#e1f5ff
    style PHOENIX_API fill:#fff4e1
...
```mermaid
graph TD
    A["robotPeriodic() [50Hz]"] --> B["CommandScheduler.run()"]
    B --> C["drivetrain.periodic()"]
    C --> D["setControl(currentRequest)"]
    D --> E["SwerveDrivetrain.setControl()"]
    E --> F1["Converts SwerveRequest to module states"]
    E --> F2["Optimizes module states (avoid >90° turns)"]
    E --> F3["Applies control to all 8 motors"]
    E --> F4["Updates odometry asynchronously"]
```
